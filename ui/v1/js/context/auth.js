...
const AuthProvider = (props) => {
  ...
  const [hasUserPreviouslyLoggedin, setHasUserPreviouslyLoggedin] = useLocalStoredState('hasUserPreviouslyLoggedin');
  const [isOnboardingUser, setIsOnboardingUser] = useState(!userData?.onboarding_finished_at);
  ...
  const login = useCallback(
    async (data) => {
      return axios.post('/api/login', data).then(async (response) => {
  ...
  const register = useCallback(
    async (data) => {
      return axios.post('/api/register', data).then(async (response) => {
        setUserData(response.data);
        return response;
      });
    },
  ...
  const logout = useCallback(async () => {
    return axios.post('/api/logout').then(async (response) => {
      setIsLoggedIn(false);
      return response;
    });
  }, [setIsLoggedIn]);
  ...
  const editUser = useCallback(
    async (data) => {
      return axios.put('/api/me', data).then(async (response) => {
  ...
  const finishOnboarding = useCallback(async () => {
    return axios.post('/api/me/finish-onboarding').then(async (response) => {
      setUser(response.data);
